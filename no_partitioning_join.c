 ...
 */
 ...
 /** install index-join step two for the target table*/
 int64_t 
index_hashtable(hashtable_t *ht, relation_t *relS)
 {
     uint32_t i, j;
     int64_t matches;
 /*
     ...
     */    
         ...
         //intkey_t idx = relS->tuples[i].key ;
         
         //intkey_t b = relR->tuples[idx-1].key;

         int32_t idx = relS->tuples[i].key/BUCKET_SIZE;
         int32_t offset = relS->tuples[i].key%BUCKET_SIZE;
         //printf("relation: R: %d, S: %d\n", b, idx);
...
int64_t
INDEX_st(relation_t *relR, relation_t *relS, int nthreads){
    hashtable_t * ht;
    int64_t result = 0;
#ifndef NO_TIMING
    struct timeval start, end;
    ...
    int num_ht_buckets = relR->num_tuples/BUCKET_SIZE + 1;
    ht->buckets = (bucket_t*) calloc(num_ht_buckets, sizeof(bucket_t));
    
    for(int i=0; i < relR->num_tuples; i++){       
        int32_t idx = relR->tuples[i].key/BUCKET_SIZE;
        int32_t offset = relR->tuples[i].key%BUCKET_SIZE;
        /* copy the tuple to appropriate hash bucket */
        /* if full, follow nxt pointer to find correct place */
    ...
    result = index_hashtable(ht, relS);
...
NPO_st(relation_t *relR, relation_t *relS, int nthreads)
{
    ...
    result = probe_hashtable(ht, relS);
    //result = index_hashtable(relR, relS);
 ...
 */
 ...
 /** plan to install index-hash join step one here, extract an index here*/
void 
