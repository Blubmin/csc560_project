...
static struct algo_t algos [] = 
  {
      ...
      {"INDEX_st", INDEX_st}, /* INDEX single threaded */
      {{0}, 0}
  };
...
main(int argc, char ** argv)
{
    ...
    cmd_params.algo     = &algos[6]; /* originally use 0:PRO, now switch to 6: INDEX_st */
    ...
    if(cmd_params.fullrange_keys) {
        ...
        create_relation_pk_R(&relR, cmd_params.r_size);
    }
    printf("OK \n");
    ...
    for(int i = 0; i< relR.num_tuples; i++){
        printf("in reaR: index %d, key: %d, payload: %d\n", i, relR.tuples[i].key, relR.tuples[i].payload);
    }
    printf("\n\n");
    for(int i = 0; i< relS.num_tuples; i++){
        printf("in reaS: index %d, key: %d, payload: %d\n", i, relS.tuples[i].key, relS.tuples[i].payload);
    }
*/
